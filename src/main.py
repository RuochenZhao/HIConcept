...
def set_args():
    ...
    parser.add_argument("--n", dest="n", type=int, default=60000)
    ...
    parser.add_argument("--without_pretraining", action='store_true') # whether to use the pre_trained version of BERT
    ...
    parser.add_argument("--hidden_dim", dest="hidden_dim", type=int,
                        default=500) 
    # TOPIC MODEL
    parser.add_argument("--freeze", action='store_false') #defaulted to true
    ...
    # POST-HOC ANALYSIS
    parser.add_argument("--eval_causal_effect", action='store_true') #should be true
    parser.add_argument("--causal_effect_filename", dest="causal_effect_filename", type=str,
                        default='causal_effect')
    parser.add_argument("--postprocess", action='store_true') #should be true
    parser.add_argument("--one_correlated_dimension", action='store_true') #should be true
...
def main(args):
    ...
    logger.info(f'args.train_topic_model: {args.train_topic_model}')
    ...
    if args.dataset == 'toy':
        ...
        if (not (os.path.exists('x_data.npy') and os.path.exists('y_data.npy') and os.path.exists('concept_data.npy'))) or args.generate_toy_data:
            ...
            # create dataset
            toy_helper.create_dataset(args.logger, args.n, args.cov, p = args.p)
        ...
        if 'bert' in args.model_name:
            ...
            if args.without_pretraining:
                args.model_save_dir = '../models/' + args.dataset + '/' + args.model_name +'/imdb_weights_wopretraining/' 
            else:
    ...
    if not args.divide_bert:
        ...
        if args.dataset == 'toy':
            logger.info(f'y_pred_train: {y_pred_train[:10]}')
            pred = y_pred_train.round().squeeze()
        else:
            logger.info(f'y_pred_train.max(1).indices[:10]: {y_pred_train.max(1).indices[:10]}')
            pred = y_pred_train.max(1).indices
        logger.info(f'pred: {pred}')
        logger.info(f'y_train[:10]: {y_train[:10]}')
        y = y_train
    ...
    if args.train_topic_model or args.eval_causal_effect or args.interpret or args.PDP or (args.visualize!=None) or args.visualize_wordcloud:
        ...
        logger.info('TESTING {} METHOD NOW'.format(args.overall_method))
        if not args.divide_bert:
    ...
    if args.interpret:
        ...
        concept_analysis(f_train, x_train, topic_vector, save_file, args.model_name, args.logger)
        logger.info('concept intepretation finished')
    if args.PDP:
        logger.info('TESTING {} METHOD NOW'.format(args.overall_method))
        ...
        make_PDP(f_train, classifier, weight_softmax, save_file, device, args.overall_method, args.model_name, args.logger, rge = [-2, 2])
        logger.info('generated PDP graph')

    if (args.visualize!=None) or args.visualize_wordcloud:
        logger.info('TESTING {} METHOD NOW'.format(args.overall_method))
        if args.dataset == 'toy':
            toy_helper.visualize_model(args.logger, x_train, f_train, topic_vector, args.graph_save_folder, args.n_concept, args.overall_method, 
